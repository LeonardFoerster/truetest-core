...
- **Phase 0 — Safe Tiny-Size Mainnet Futures**: In active execution (0/15 qualifying as of 2026; collection paused on monte-carlo branch per reports/phase0/ + cross-refs while MC is priority — gates/ritual unchanged). Collecting 15+ qualifying sessions across volatility regimes with zero unexplained drift. Full artifacts (binary log + QuestDB + notes) required. Tracker in [`reports/phase0/`](reports/phase0/); current ritual + template in [`prod.md`](prod.md); printable SOP planned in `docs/operations/futures-phase0-operator-sop.md` (Doc Phase 1; explicit "current details live in prod.md" per CLAUDE rules).
