...
**Status (2026)**: 0/15 qualifying sessions. Scripts and templates ready. Monte Carlo simulation capabilities (integrated from the monte-carlo branch, now mainline) are available for research and strategy robustness (see `docs/governance/03-todo.md` MC-* and `docs/reference/01-instructions.md`). Phase 0 evidence collection remains at zero sessions; the requirements and ritual defined in `docs/governance/01-prod.md` are unchanged. (Language sync post monte-carlo merge.)
