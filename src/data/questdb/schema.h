...
// Per-run DDL statements, prefixed with run_tag. ttl_days <= 0 disables TTL.
std::vector<std::string> per_run_ddls(const std::string& run_tag,
                                      int ttl_days = 0);

// Convenience - all statements in the order they should be issued.
...
inline constexpr const char* kTableEvents        = "events";
