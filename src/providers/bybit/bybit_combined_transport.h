...
// Multi-subscribe public WS (e.g. trade + orderbook.50) on one linear path.
class BybitCombinedTransport : public IDataTransport
{
public:
    BybitCombinedTransport(const std::string& symbol,
    ...
    bool open() override
    {
        try
        {
            if (streams_.empty())
            {
                std::cerr << "BybitCombinedTransport: no streams to subscribe\n";
                return false;
            }
            ...
            {
                std::cerr << "BybitCombinedTransport: SNI setup failed\n";
                return false;
            }
        ...
        {
            std::cerr << "BybitCombinedTransport: open failed: " << e.what()
                      << "\n";
            return false;
        }
    }
    ...
    bool read_frame_blocking(std::string_view& out) override
    {
        ...
        while (!stopped_.load())
        {
            ...
            catch (const beast::system_error& se)
            {
                ...
                std::cerr << "BybitCombinedTransport: websocket "
            ...
            {
                std::cerr << "BybitCombinedTransport: read error: " << e.what()
                          << "\n";
                open_ = false;
                ...
                if (fatal_cb_)
                {
                    char buf[160];
                    std::snprintf(buf, sizeof(buf),
                                  "bybit combined WS lost: %s", e.what());
                    fatal_cb_(buf);
                    stopped_ = true;
                    return false;
                }
...
private:
    ...
    void maybe_send_ping(bool force = false)
    {
        ...
        {
            std::cerr << "BybitCombinedTransport: ping failed: " << e.what()
                      << "\n";
        }
    }
...
using BybitCombinedTransport = bybit::BybitCombinedTransport;
