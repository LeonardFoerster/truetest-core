...
// Venue-local paper + hybrid execution for Gate non-live modes.
// Mirrors BitgetHybridExecutor so Gate compiles without HAS_BINANCE.
...
class GatePaperExecutor : public IExecutionAdapter
{
public:
    GatePaperExecutor() = default;
    ...
    void set_fee_model(std::shared_ptr<IFeeModel> fm)
    {
        fee_model_ = std::move(fm);
    }
    ...
    void submit_order(const order_event& o) override
    {
        ...
        {
            std::cout << "  [PAPER] " << side << " " << o.get_quantity()
                      << " " << o.get_symbol() << " @ " << px << "\n";
        }
        ...
        if (o.get_order_type() == order_type::market && last_price_ > 0)
        {
            ...
            pending_fills_.emplace_back(o.get_earliest_eligible_ts(),
                                        o.get_symbol(), o.get_order_id(),
                                        o.get_side(), o.get_quantity(),
                                        last_price_, commission);
            ...
            if (o.get_recv_ns() > 0)
            {
                const int64_t now_ns =
                    std::chrono::duration_cast<std::chrono::nanoseconds>(
                        ...
                        .count();
                pending_fills_.back().set_latency_ns(now_ns
                                                     - o.get_recv_ns());
            }
        }
    }
    ...
    bool cancel_order(uint64_t order_id) override
    {
        auto it = std::remove_if(
            pending_fills_.begin(), pending_fills_.end(),
            [order_id](const fill_event& f) {
                return f.get_order_id() == order_id;
            });
...
class GateHybridExecutor : public IExecutionAdapter
{
public:
    GateHybridExecutor(
        std::shared_ptr<GatePaperExecutor> paper_exec,
    ...
    void submit_order(const order_event& o) override
    {
        ...
        if (latency_model_)
            order_latencies_[o.get_order_id()] =
                latency_model_->get_order_latency();
    ...
    bool poll_fills(std::vector<fill_event>& out) override
    {
        ...
        for (auto& f : inner)
        {
            ...
            delayed_fills_.push_back(
                {std::move(f), f.get_timestamp() + latency});
        }

        bool released = false;
        auto new_end = std::remove_if(
            delayed_fills_.begin(), delayed_fills_.end(),
            [&](delayed_fill& df) {
    ...
    bool modify_order(uint64_t order_id, double new_price,
    ...
    void on_l2_update(const std::string& symbol, order_side side,
...
private:
    ...
    std::shared_ptr<GatePaperExecutor> paper_;
