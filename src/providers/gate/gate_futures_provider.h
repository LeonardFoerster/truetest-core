...
public:
    ...
    // Phase 0: no network. Marks lifecycle open so registry/unit tests can
    // exercise construction without TLS. Phase 1 wires real transport.
    bool open() override
    {
        if (lifecycle_ == lifecycle::open) return true;
