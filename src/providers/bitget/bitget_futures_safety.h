...
// Startup advisories for Bitget UTA futures (cold path).
// Input: GET /api/v3/position/current-position body (or equivalent list).
// Warnings only — refusal is operator-driven via margin_type_strict
// (already enforced in provider open from account/settings).
    ...
    double liquidation_warn_pct)
{
    ...
    auto arr = detail::extract_array(position_json, "list");
    if (arr.empty())
        arr = detail::extract_array(position_json, "data");
    ...
    auto consider = [&](std::string_view obj) {
        ...
        auto size_sv = extract_sv_string(obj, "total");
        if (size_sv.empty())
            size_sv = extract_sv_number(obj, "total");
        if (size_sv.empty())
        ...
        auto pos_side = extract_sv_string(obj, "posSide");
        if (pos_side.empty())
            pos_side = extract_sv_string(obj, "holdSide");
        const bool is_short =
            pos_side == "short" || pos_side == "SHORT"
            || pos_side == "sell" || pos_side == "SELL";
        // one-way signed size
        double signed_qty = size;
        if (is_short)
            signed_qty = -size;
        else if (pos_side.empty())
        {
        ...
        if (check_margin)
        {
            auto mt = extract_sv_string(obj, "marginMode");
            if (mt.empty())
        ...
        if (check_liq)
        {
            ...
            auto liq_sv = extract_sv_string(obj, "liquidationPrice");
            if (liq_sv.empty())
                liq_sv = extract_sv_number(obj, "liquidationPrice");
            if (liq_sv.empty())
                liq_sv = extract_sv_string(obj, "liqPrice");
            if (liq_sv.empty())
    ...
    auto data_obj = detail::extract_object(position_json, "data");
    if (!data_obj.empty())
        consider(data_obj);
    return out;
}

// Strict margin refusal helper (mirrors Binance). Liq distance is advisory-only.
