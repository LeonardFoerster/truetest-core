...
TEST(ExitManager, OnBar_TrailingDoesNotLookAheadWithinBar)
{
    ...
    auto r = m.on_bar("X", /*open=*/105.0, /*low=*/104.0, /*high=*/110.0, /*close=*/105.0, t0);
    EXPECT_TRUE(r.empty()) << "same-bar trail+SL is look-ahead";
    EXPECT_EQ(m.armed_count(), 1u);

    // Next bar: trail is now active from prior high; low through 108.9 fires
    // at the stop level (anchored fill), not the bar extreme.
    auto r2 = m.on_bar("X", /*open=*/109.0, /*low=*/108.5, /*high=*/109.0, /*close=*/108.8, t0);
    ASSERT_FALSE(r2.empty());
    EXPECT_DOUBLE_EQ(r2[0].get_price(), 108.9);
}
