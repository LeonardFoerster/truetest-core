...
**Status note (2026, integrated from monte-carlo branch)**: Lower-priority work (see root `docs/governance/03-todo.md` A-01..A-07 or docs/todos/07-A-adaptive-hybrid.md#A-01 for verbatim). MC simulation (now mainline) exercises strategies including this one for robustness (MC-05; see docs/todos/03-MC-simulation.md). Strategy is registered + L2 dispatch present (with `LIVE_SAFETY_CCB_APPROVED` comment in engine) + test + example JSON; usable for backtests/MC experiments. However, code contains multiple "compilable demo" / "simplified decision" / "real version" / "placeholder" / "v1" / always-on `enable_onchain_mock` caveats (see `src/strategy/adaptive_hybrid_strategy.cpp` + .h + config.h). On-chain is mock only; no `take_pending_exit_intents`; simplified RiskValidator/equity/L2 paths. Full hot-path polish deferred. See code comments + root `docs/governance/03-todo.md` A-* (or docs/todos/) for exact items. Spec here remains useful reference.
