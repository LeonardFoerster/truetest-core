# QuestDB Multi-Week Shadow / Live Run Hardening Guide (HISTORICAL / ARCHIVED)

**Note (post-2026 consolidation)**: This is a historical phased implementation log from the `database` branch (v0.5; detailed AI prompts + exact edit locations for Phases 0-5). Most phases have landed (time-based flush, --persist-strict + fallback + health, richer schema/events, scripts, soak ritual, funding table, etc. - see current `docs/reference/03-db.md` for authoritative schema + reliability + queries + ritual, `docs/reference/01-instructions.md` MC/QuestDB sections, root `docs/governance/03-todo.md` H-* items (or docs/todos/08-H-persistence-observability.md), and code in `src/data/questdb/` + engine). Value extracted; retained for audit. Do not treat as current spec.

**Original header below** (for reference):
...
This is **H-01** in todo.md (see docs/governance/03-todo.md or docs/todos/08-H-persistence-observability.md#H-01) and a gate in prod.md.
...
Implement `--persist-strict` + local fallback (addresses todo.md H-01 / docs/todos/08-H-persistence-observability.md#H-01 and docs/governance/01-prod.md Phase 1 gate).
...
- Add `QuestDB:` or `persist:` label to relevant items in docs/governance/03-todo.md / docs/todos/08-....md .
...
| P2 | Add `--persist-strict` + local ILP fallback file | 2 | Addresses H-01 (see docs/todos/08-H-persistence-observability.md#H-01) |
