...
portfolio::portfolio() : cash_(100000.0) {}
...
void portfolio::on_fill(const fill_event& fill)
{
    ...
    else if (fill.get_side() == order_side::sell && pos.qty > 0)
    {
        int sell_qty = std::min(fill.get_filled_quantity(), pos.qty);

        // Compute average entry price for the portion being sold
        double avg_entry = (pos.qty > 0) ? pos.cost_basis / pos.qty : 0.0;
...
void portfolio::print_summary() const
{
    ...
    {
        if (pos.qty > 0)
            std::cout << "Position " << sym << ": " << pos.qty << " shares" << std::endl;
    }
