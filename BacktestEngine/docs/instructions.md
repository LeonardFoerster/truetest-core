...
|-------------------|---------------------|-----------------------------------------------------|
| Provider          | `IProvider`         | Local (file-based backtesting)                       |
| Data transport    | `IDataTransport`    | File (batch), WebSocket (streaming, future)          |
| Data parser       | `IDataParser<T>`    | CsvBarParser, CsvTickParser                          |
| Data bridge       | `DataBridge<T>`     | Batch + streaming orchestrator (transport → parser → sink) |
...
Enables performance profiling, copy detection, memory tracking, and hardware
diagnostics via Abseil Logging. Abseil is fetched automatically via FetchContent.
...
When `ENABLE_DEBUG` is OFF (the default): zero overhead — no vtable indirection,
no branches, no extra storage, no Abseil dependency.

When enabled, the engine produces a full debug report after each run:

- **Hardware snapshot** — CPU model, core count, cache sizes, NUMA nodes, RAM, isolated cores
- **Per-stage latency** — wall-clock timing for each pipeline stage (market create, strategy, orderbook, fill processing, ring publish, risk check, MM replenish, stop check, pending drain)
- **Copy/move report** — tracks copies vs moves on hot-path types (`market_event`, `order_event`, `fill_event`, `tick_event`, `tick_record`, `bar_record`). Non-zero copies are flagged at WARNING severity
- **Memory usage** — RSS, virtual memory, peak RSS, and heap at start/end with deltas
- **Thread utilization** — busy/idle ratio, poll hit rate per worker thread
- **Ring buffer diagnostics** — push/pop counts, drops, high-water mark per ring

#### Runtime log filtering

Abseil supports runtime severity filtering without recompiling:

```bash
./build/truetest --minloglevel=0   # show everything (default)
./build/truetest --minloglevel=1   # warnings and errors only
./build/truetest --minloglevel=2   # errors only
```

A log file (`truetest_debug.log`) is also written alongside stderr output.
...
Instead of the interactive TUI, you can run a backtest entirely from the
command line using the provider system:

| Flag                       | Description                                           |
|----------------------------|-------------------------------------------------------|
| `--provider <name>`        | Use a registered provider (skips TUI). Currently: `local` |
| `--path <file>`            | Data file path (required for `local` provider)        |
| `--format bar\|tick`       | Data format: `bar` (OHLCV, default) or `tick`         |
| `--strategy <name>`        | Strategy: `mean-reversion` (default), `sma`, `ma-crossover` |
| `--sma-period <N>`         | SMA lookback period (default: 20)                     |
| `--fee fixed\|tiered`      | Fee model (default: zero fees)                        |
| `--fee-value <N>`          | Fixed fee amount (used with `--fee fixed`)            |
| `--maker-rate <N>`         | Maker rate (used with `--fee tiered`)                 |
| `--taker-rate <N>`         | Taker rate (used with `--fee tiered`)                 |

Example — run a bar backtest via provider:

```bash
./build/truetest --provider local --path market_data.csv --strategy sma --sma-period 5
```

Example — run a tick backtest with tiered fees:

```bash
./build/truetest --provider local --path ticks.csv --format tick --fee tiered --maker-rate 0.001 --taker-rate 0.002
```
...
| FileTransport              | 3     | Batch fallback, streaming flag, request_stop     |
| MockStreamingTransport     | 5     | Blocking read, stop/unblock, data ordering       |
| MockBatchTransport         | 2     | Non-streaming transport basics                   |
| DataBridge                 | 8     | Batch load (bar/tick/empty/malformed), streaming  |
| ProviderEvent              | 11    | Event variant, conversions, sink dispatch         |
| ProviderRegistry           | 7     | Registry CRUD, factory, available providers       |
| LocalProvider              | 7     | Capabilities, transport access, open/close        |
...
| `ENABLE_DEBUG`      | `OFF`   | Build with performance instrumentation (Abseil)  |
...
cmake -B build -DENABLE_DEBUG=ON                   # with debug instrumentation
...
│   ├── test_provider_transport.cpp  # transport interface tests
│   ├── test_data_bridge.cpp         # batch + streaming bridge tests
│   ├── test_provider_event.cpp      # event variant + conversion tests
│   ├── test_provider_registry.cpp   # registry + LocalProvider tests
│   ├── test_helpers/
│   │   └── mock_transport.h         # MockStreamingTransport, MockBatchTransport
│   └── fixtures/
│       ├── sample_ohlcv.csv
│       └── sample_ticks.csv
└── BacktestEngine/src/
    ...
    ├── providers/                   # provider architecture (data transport + parsing)
    │   ├── transport.h             # IDataTransport: batch + streaming interface
    │   ├── parser.h                # IDataParser<T>: line-to-record conversion
    │   ├── data_bridge.h           # DataBridge<T>: transport → parser → sink orchestrator
    │   ├── provider.h              # IProvider: bundled data feed + execution per venue
    │   ├── provider_event.h        # provider::event variant (bar, tick, l2, status)
    │   ├── provider_convert.h      # bar_record/tick_record ↔ provider::bar/tick
    │   ├── provider_sink.h         # event_sink for DataBridge<provider::event>
    │   ├── provider_registry.h     # ProviderRegistry singleton + REGISTER_PROVIDER macro
    │   ├── local/
    │   │   ├── file_transport.h    # FileTransport: reads lines from local files
    │   │   ├── csv_parser.h        # CsvBarParser, CsvTickParser, sink functions
    │   │   ├── local_provider.h    # LocalProvider: file transport, no execution
    │   │   └── local_register.cpp  # self-registers "local" provider
    │   ├── binance/
    │   │   └── README.md           # stub — future Binance provider
    │   ├── polymarket/
    │   │   └── README.md           # stub — future Polymarket provider
    │   └── metatrader/
    │       └── README.md           # stub — future MetaTrader EA provider
    ├── debug/                      # performance instrumentation (opt-in)
    │   ├── debug_log.h             # Abseil log wrapper, severity macros, FileSink
    │   ├── hardware_info.h/.cpp    # CPU, cache, RAM, NUMA detection (Linux sysfs)
    │   ├── copy_tracker.h          # CRTP mixin — counts copies/moves per type
    │   ├── stage_timer.h/.cpp      # RAII per-stage latency measurement
    │   ├── memory_info.h/.cpp      # RSS, VM, heap snapshots from /proc
    │   ├── thread_stats.h          # busy/idle/poll stats per worker thread
    │   ├── ring_stats.h            # push/pop/drop/HWM tracking per ring buffer
    │   └── debug_report.h/.cpp     # orchestrator — emits full report at run end
    └── utils/
